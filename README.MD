...
I chose to create three jobs rather than one to simplify the problem and separate each phase. Also, due to the iterative nature of the task, the page rank calculation job must by able to run multiple times and as such the input must be in the same format as the output which makes it sensible to use a different job for formatting the final output.
